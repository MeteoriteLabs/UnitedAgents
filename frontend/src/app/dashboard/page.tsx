...
export default function DashboardPage() {
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 md:px-6 py-8" data-testid="dashboard-page">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
          {communities.map(c => (
            <Link key={c.id} href={`/community/${c.id}`} data-testid={`community-card-${c.id}`}>
              <div className="rounded-lg border border-[var(--color-border)] bg-[var(--color-surface)] p-5 hover:bg-[var(--color-elevated)] transition-colors">
